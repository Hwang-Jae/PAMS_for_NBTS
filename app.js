...
function bindEvents() {
    ...
    $('#print-bulk-label-btn').onclick = () => window.print();// 이 줄만 남깁니다.
...
function renderBulkLabels(assets) {
    ...
    /*assets.forEach((asset, index) => {
        ...
        const qrId = `bulk-qr-${index}`;
        container.innerHTML += `
            <div class="w-full border-2 border-dashed border-slate-400 p-1 bg-white" style="page-break-inside: avoid;">
                <div class="flex items-start space-x-2">
                    <div id="${qrId}" class="flex-shrink-0 p-1 border"></div>
                    
                    <div class="flex flex-col w-full">
                        <img src="logo_NBTS.gif" alt="Logo" class="h-8 w-auto mb-3">
                        <p class="text-base font-bold text-black break-words leading-tight mb-3">${asset.cmf_2}</p>
                        <p class="text-[10px] text-black font-bold mt-1 pt-1 border-t border-slate-200">
                            WARNING: Do not damage or remove this label.
                        </p>
                    </div>
                </div>
            </div>
    ...
    // QR 코드를 생성하되, 브라우저가 멈추지 않도록 <canvas> 대신 <img>로 변환합니다.
    qrInstances.forEach(qr => {
        try {
            // 1. 임시 div에 QR코드를 (canvas로) 생성합니다.
        ...
        if (canvas) {
            // 3. 캔버스 데이터를 이미지(Data URL)로 변환합니다.
            const dataUrl = canvas.toDataURL();
            
            // 4. 실제 DOM의 QR코드 위치(placeholder)를 찾습니다.
            const placeholder = document.getElementById(qr.id);
            
            if (placeholder) {
                // 5. 캔버스 대신 <img> 태그를 삽입합니다.
                const img = document.createElement('img');
                img.src = dataUrl;
                img.style.width = '60px';
                img.style.height = '60px';
                
                placeholder.innerHTML = ''; // 기존 내용 삭제
        ...
        console.error(`QR 생성 실패 (ID: ${qr.id}):`, e);
        const placeholder = document.getElementById(qr.id);
        if (placeholder) placeholder.innerText = "QR 오류";
        }
    });
...
// =========================================
// Zebra Browser Print 함수
// =========================================

/**
 * 2. Zebra 프린터 상태 확인 및 기본 프린터 가져오기
 */
async function setupAndGetPrinter() {
    return new Promise((resolve, reject) => {
        // Zebra Browser Print API 초기화
        Zebra.BrowserPrint.init(() => {
            // 기본 장치 가져오기
            Zebra.BrowserPrint.getDefaultDevice("printer", 
                (printer) => {
                    console.log("기본 프린터 확인:", printer);
                    resolve(printer); // 프린터 객체 반환
                },
                (errorMsg) => {
                    reject(`프린터를 찾을 수 없습니다: ${errorMsg}`);
                }
            );
        }, (errorMsg) => {
            reject(`Browser Print 초기화 실패: ${errorMsg}. 프로그램이 실행 중인지 확인하세요.`);
        });
    });
}
 ...
 * 3. ZPL 생성 및 전송 함수
 */
async function handleZebraBulkPrint() {
    showLoading('Zebra 프린터로 전송 중...');
    
    // [수정] 전역 state 객체에서 선택된 자산 목록(Set)을 기반으로
    // 실제 자산 데이터를 필터링합니다.
    const assetsToPrint = state.assets.filter(a => state.selectedAssets.has(a.cmf_2));
    ...
    try {
        const printer = await setupAndGetPrinter();
        
        // ZPL (Zebra Programming Language) 문자열 생성
        let combinedZpl = '';
        
        // [수정] 필터링된 자산 배열(assetsToPrint)을 사용합니다.
        assetsToPrint.forEach(asset => {
            // ZPL은 라벨 디자인에 맞춰 직접 코딩해야 합니다.
            // 아래는 예시이며, 좌표(FO)와 폰트 크기, QR 크기를
            // 사용하는 라벨 용지(예: 4x2인치)에 맞게 조정해야 합니다.
            
            const qrData = asset.cmf_2; // QR 코드에 담을 데이터 (관리 코드)
            const line1 = asset.cmf_2;  // 텍스트 라인 1 (관리 코드)
            const line2 = asset.product_name; // 텍스트 라인 2 (자산명)
            ...
            combinedZpl += `
                ...
                ^FO25,25^A0N,30,30^FD자산 관리 코드^FS
                ^FO25,45^A0N,45,45^FD${line1}^FS
                ^FO25,75^A0N,28,28^FD${line2}^FS

                ^FO225,25^BQN,2,4^FDQA,${qrData}^FS

                ^FO25,175^A0N,20,20^FD(주)엔비티에스 - 자산라벨 - 훼손주의^FS
                ^XZ
            `;
            // ^XA (시작), ^XZ (종료)는 각 라벨마다 필요합니다.
        });

        // 프린터로 ZPL 전송
        printer.send(combinedZpl, 
            () => {
                hideLoading();
                alert('인쇄 작업이 Zebra 프린터로 전송되었습니다.');
                // ... (선택 해제 등 후속 처리)
            }, 
            (error) => {
        ...
        hideLoading();
        console.error("Zebra 인쇄 오류:", error);
        alert(`인쇄 실패: ${error}\n\nZebra Browser Print 프로그램이 실행 중인지, 프린터가 연결되었는지 확인하세요.`);
    }
}
